  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/user/login", {
        ...
        localStorage.setItem("userId", res.data.find._id);
        navigate("/dashboard")
      })
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <a href="#" className="font-medium text-[#2e7d5c] hover:text-[#4b9b6e]">
                Forgot password?
              </a>
            </div>
          </div>
          ...
          <button
            ...
            className={`group relative w-full flex justify-center py-3 px-4 border border-transparent 
                      ...
                      transition-all duration-300 ${isLoading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            {isLoading ? (
          ...
          <p className="mt-6 text-center text-sm text-gray-600 cursor-pointer">
            ...
            <p className="font-medium text-[#4b9b6e] hover:text-[#6bbf84]" onClick={()=> navigate("/")}>
              Sign up now
            </p>
          </p>
        </form>
